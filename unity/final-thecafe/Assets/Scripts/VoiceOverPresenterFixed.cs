...
namespace Yarn.Unity
{
    ...
    /// </summary>
    public sealed class VoiceOverPresenterFixed : DialoguePresenterBase
    {
...
#pragma warning restore CS8618
        ...
        public override async YarnTask RunLineAsync(LocalizedLine dialogueLine, LineCancellationToken lineCancellationToken)
        {
            ...
            if (!lineCancellationToken.IsNextContentRequested && waitTimeAfterLineComplete > 0)
            {
                ...
                {
                    Debug.Log($"Line was interrupted while waiting after line {dialogueLine.TextID} complete, skipping remaining wait time.");
                    return;
                }   

                // if (lineCancellationToken.IsNextContentRequested)
                // {
                //     Debug.Log($"Line was interrupted while waiting after line {dialogueLine.TextID} complete, skipping remaining wait time.");
                //     return;
                // }
            ...
            // Debug.Log("lineCancellationToken details: " +
            //     $"IsNextContentRequested={lineCancellationToken.}, " +
            //     $"IsNextContentRequested={lineCancellationToken.IsNextContentRequested}, " +
            //     $"IsHurryUpRequested={lineCancellationToken.IsHurryUpRequested}");
